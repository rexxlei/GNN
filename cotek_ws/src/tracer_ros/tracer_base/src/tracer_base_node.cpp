...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        {
            std::cout << "Detected protocol: UNKNOWN, using AGX_V2 as default" << std::endl;
            ROS_WARN("Could not detect protocol version, defaulting to AGX_V2");
            tracer = std::unique_ptr<TracerRobot>(
            new TracerRobot(ProtocolVersion::AGX_V2));
        }
